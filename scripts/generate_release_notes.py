...
def get_merged_prs(limit: int = 100) -> dict[int, PullRequest]:
    ...
    return {pr.number: pr for pr in msgspec.json.decode(result.stdout, type=list[PullRequest])}
...
def categorize_entries(entries: list[CategorizedEntry]) -> dict[str, list[CategorizedEntry]]:
